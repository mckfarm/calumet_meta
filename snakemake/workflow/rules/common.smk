...
def get_rules(wildcards):
    ...
    if config["assembly"]:
        all_rules.append("results/megahit/final.contigs.fa")

        all_rules.append("results/megahit_quast/report.html")

        all_rules = all_rules + expand(
            "results/megahit_coverage/{sample}/{sample}.sorted.bam", sample=sample_sheet["sample_name"])
